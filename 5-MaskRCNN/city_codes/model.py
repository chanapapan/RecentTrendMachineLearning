    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            ...
            placeholder[gt_y1:gt_y2, gt_x1:gt_x2] = \
                np.transpose(np.round((numpy.array(Image.fromarray(class_mask.astype(float)).resize((gt_h, gt_w))) /255).astype(bool)))
        ...
        '''
        mask = numpy.array(Image.fromarray(m.astype(float)).resize(config.MASK_SHAPE)).astype(np.float32) / 255.0
